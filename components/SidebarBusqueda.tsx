...
export default function SidebarBusqueda({
  ...
  return (
    <div className={`absolute left-0 top-0 h-full z-10 flex flex-col bg-white ${selectedWorkspaceId ? "w-1/3 border-r border-gray-200 pointer-events-auto" : "w-full pointer-events-none"}`}>

      {selectedWorkspaceId && (
        searchMinimized ? (
          <div className="h-[56px] min-h-[56px] flex-none bg-gray-50/80 backdrop-blur-sm px-4 py-2.5 flex items-center justify-between border-b border-gray-200 transition-all duration-300 animate-in fade-in pointer-events-auto">
          ...
          <div className="flex-[1] flex flex-col min-h-0 bg-gray-50/50 p-4 transition-all duration-300 animate-in fade-in pointer-events-auto">
      ...
      <div className="pointer-events-auto flex-[3] relative w-full min-h-0">
        {children}
      </div>
    </div>
  );
